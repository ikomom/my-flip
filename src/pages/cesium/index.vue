...
import {
  Cartesian3,
  Math as CesiumMath,
  GeoJsonDataSource, HeadingPitchRoll, Ion, Terrain, Transforms, Viewer,
} from 'cesium'
...
Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIxOTkxZDY0Ny02NGE3LTRjNjgtYThlNy1mNjVmNTMxZWIwMmEiLCJpZCI6MzUzMTEsImlhdCI6MTc0MTE1MTcxNH0.bglSD_DCwqvh_A64oHxoxUtT8VuXRwN0GYSeGwR6yBU'
let viewer: Viewer
...
function addPlane() {
  ...
  viewer.camera.flyTo({
    destination: Cartesian3.fromDegrees(121, 23.4, 240000),
    orientation: {
...
// These are all the radar points from this flight.
  ...
  // SampledPositionedProperty 存储雷达样本序列中每个样本的位置和时间戳。
  const positionProperty = new Cesium.SampledPositionProperty()

  for (let i = 0; i < flightData.length; i++) {
    const dataPoint = flightData[i]

    // 声明此单个样本的时间并将其存储在新的 JulianDate 实例中。
  ...
  // 创建一个实体，用一条线可视化整个雷达样本系列，并添加一个沿样本移动的点。
